...
data "archive_file" "uuid" {
  type = "zip"

  source_dir = "../layers/uuid"
  output_path = "../build/layers/uuid.zip"
}

resource "aws_s3_object" "uuid" {
  bucket = aws_s3_bucket.lambda_bucket.id

  key = "uuid.zip"
  source = data.archive_file.uuid.output_path
  etag = filemd5(data.archive_file.uuid.output_path)
}
...
# request create
data "archive_file" "lambda_request_create" {
  type = "zip"

  source_file = "../dist/request/request_create.js"
  output_path = "../build/request/request_create.zip"
}

resource "aws_s3_object" "lambda_request_create" {
  bucket = aws_s3_bucket.lambda_bucket.id

  key = "request_create.zip"
  source = data.archive_file.lambda_request_create.output_path
  etag = filemd5(data.archive_file.lambda_request_create.output_path)
}

# request findAll
data "archive_file" "lambda_request_find_all" {
  type = "zip"

  source_file = "../dist/request/request_findAll.js"
  output_path = "../build/request/request_findAll.zip"
}

resource "aws_s3_object" "lambda_request_find_all" {
  bucket = aws_s3_bucket.lambda_bucket.id

  key = "request_findAll.zip"
  source = data.archive_file.lambda_request_find_all.output_path
  etag = filemd5(data.archive_file.lambda_request_find_all.output_path)
}

# request find one
data "archive_file" "lambda_request_find_by_id" {
  type = "zip"

  source_file = "../dist/request/request_findOne.js"
  output_path = "../build/request/request_findOne.zip"
}

resource "aws_s3_object" "lambda_request_find_by_id" {
  bucket = aws_s3_bucket.lambda_bucket.id

  key = "request_findById.zip"
